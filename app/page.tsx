    ...
    <div className="relative flex flex-col h-screen max-w-md mx-auto bg-white font-sans overflow-hidden">
      ...
      <header className="px-4 pt-12 pb-2 flex justify-between items-center bg-white/80 backdrop-blur-md sticky top-0 z-[40]">
        ...
        <div className="relative">
          ...
          {/* 팝업 메뉴 (z-index 50으로 최상단 고정) */}
          {isMenuOpen && (
            <>
              <div className="fixed inset-0 z-[45] bg-black/5" onClick={() => setIsMenuOpen(false)} />
              <div className="absolute right-0 mt-2 w-48 bg-white/95 backdrop-blur-2xl rounded-2xl shadow-2xl border border-gray-200 py-1 z-[50] overflow-hidden shadow-black/10">
                <button 
                ...
                <button 
                  ...
                  className="w-full px-4 py-3.5 flex items-center justify-between active:bg-gray-200 transition-colors border-b border-gray-100"
                >
                  <span className="text-[16px] font-medium">사진첩</span>
                ...
                >
                  <span className="text-[16px] font-medium">타임라인</span>
      ...
      <main className="flex-1 overflow-y-auto">
        <div onClick={() => router.push('/chat')} className="flex items-center px-4 py-3 active:bg-gray-100 transition-colors cursor-pointer group">
          <div className="w-14 h-14 rounded-full bg-gray-200 overflow-hidden border border-black/5 shrink-0">
            {profile?.avatar_url ? <img src={profile.avatar_url} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gray-300 flex items-center justify-center text-gray-500 font-bold">{profile?.character_name?.charAt(0) || '히'}</div>}
          </div>
          <div className="ml-3 flex-1 border-b border-gray-100 pb-3 group-last:border-none">
            <div className="flex justify-between items-baseline mb-0.5">
              ...
              <span className="text-[14px] text-gray-500">{lastMessage ? new Date(lastMessage.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}</span>
            </div>
            <div className="flex justify-between items-center">
              <p className="text-[14px] text-gray-500 line-clamp-2 pr-4">{lastMessage?.content || '새로운 대화를 시작해보세요.'}</p>
      ...
      <div className="p-4 flex justify-end sticky bottom-0 pointer-events-none">
        <button onClick={() => router.push('/chat')} className="pointer-events-auto w-12 h-12 bg-white rounded-full shadow-lg border border-gray-100 flex items-center justify-center text-[#007AFF] active:scale-95 transition-transform">
          <SquarePen size={24} />
        </button>
      </div>
    </div>
